      ...
      <nav className="sticky top-0 z-50 bg-black border-b border-[#ffcc66]/20 p-4">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#ffcc66]">Dhruv Sharma</h1>

            {/* Mobile Menu Button */}
            <button
              className="md:hidden hover:bg-[#ffcc66]/10 p-2 rounded-lg transition-colors"
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-black border-b border-[#ffcc66]/20">
            <div className="max-w-6xl mx-auto px-4 py-2 space-y-1">
              {["home", "about", "skills", "projects", "contact"].map((section) => (
                <button
                  ...
                  className={`w-full text-left capitalize px-3 py-2 rounded-lg transition-all duration-300 ${
                    activeSection === section 
                      ? "bg-[#ffcc66] text-black font-medium"
                      : "text-[#ffcc66] hover:bg-[#ffcc66]/10"
                  }`}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className={renderSection("home")}>
          <div className="flex flex-col items-center">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#ffcc66]">
              Dhruv Sharma
            </h2>
        ...
        <div className={renderSection("about")}>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#ffcc66]">About Me</h2>
        ...
        <div className={renderSection("skills")}>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#ffcc66]">Skills</h2>
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {skills.map((skill, index) => (
              <div key={index} className="bg-black border border-[#ffcc66]/20 rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-gray-300">{skill.name}</span>
                  <span className="text-[#ffcc66]">{skill.level}%</span>
                </div>
        ...
        <div className={renderSection("projects")}>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#ffcc66]">Projects</h2>
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project, index) => (
              <div key={index} className="bg-black border border-[#ffcc66]/20 rounded-lg p-6 hover:border-[#ffcc66] transition-colors">
                <h3 className="text-xl font-semibold text-[#ffcc66]">{project.title}</h3>
        ...
        <div className={renderSection("contact")}>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#ffcc66]">Contact</h2>
          <p className="mt-4 text-gray-300">
            I’d love to connect! Feel free to reach out via email or connect on LinkedIn.
          </p>
          <div className="flex space-x-6 mt-6">
            <a href="https://github.com/MrD0511" className="text-[#ffcc66] hover:text-[#ffcc66]/80 transition-colors">
              <Github size={24} />
            </a>
            <a href="https://linkedin.com/in/dhruvsharma005" className="text-[#ffcc66] hover:text-[#ffcc66]/80 transition-colors">
              <Linkedin size={24} />
            </a>
            <a href="mailto:sharmadhruv00005@gmail.com" className="text-[#ffcc66] hover:text-[#ffcc66]/80 transition-colors">
              <Mail size={24} />
            </a>
          </div>
        </div>
